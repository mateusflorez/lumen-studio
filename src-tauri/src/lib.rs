...
static LUMEN_LOGO_PNG: &[u8] = include_bytes!("../templates/shared/lumen_logo.png");
...
fn create_template_subject(
    ...
    fs::write(assets_dir.join("lumen_logo.png"), LUMEN_LOGO_PNG)
        .map_err(|error| format!("Não foi possível criar lumen_logo.png: {}", error))?;
...
fn resolve_generation_tool_root(workspace_root: &Path) -> Result<PathBuf, String> {
    ...
    if let Some(parent) = workspace_root.parent() {
        ...
        candidates.push(parent.join("Lumen"));
        candidates.push(parent.join("lumen"));
    }
    ...
    Err("Não encontrei as ferramentas de geração. Esperava encontrar node_modules com Marp e markdown-it no projeto base do Lumen.".to_string())
}
...
fn prepare_lesson_markdown_for_render(
    ...
    for line in content.lines() {
        ...
        if trimmed.contains("shared/lumen_logo") {
            if let Some(url) = &logo_url {
        ...
        let next_line = line
            ...
            .replace("../../shared/background.jpg", background_url.as_deref().unwrap_or(""))
            .replace("../shared/lumen_logo.png", logo_url.as_deref().unwrap_or(""))
            .replace("../../shared/lumen_logo.png", logo_url.as_deref().unwrap_or(""))
